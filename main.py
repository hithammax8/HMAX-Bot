...
def save_user(user, ref=0):
    cursor.execute(
        "INSERT OR IGNORE INTO users(user_id, name, joined_by, join_date) VALUES (?, ?, ?, ?)",
        (
            user.id,
            user.first_name,
            ref,
...
def dashboard(message):
    ...
    cursor.execute(
        "SELECT joined_by, COUNT(*) as total FROM users WHERE joined_by != 0 GROUP BY joined_by ORDER BY total DESC LIMIT 1"
    )
